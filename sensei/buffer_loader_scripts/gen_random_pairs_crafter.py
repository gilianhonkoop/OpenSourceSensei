...
if __name__ == "__main__":
    ...
    data_dir = (
        f"/mnt/qb/work/butz/bst495/logdir/p2x_crafter/crafter_p2x_restarts/working_directories/0/replay"
    )
    ...
    total_length = 0
    max_length = 1000000 # divisble by 250
    num_pairs = 500000
    ...
    # reshape the obs_gylphs to pseudo episodes (250 each)
    ...
    crafter_env = Env(size=(64, 64))
    local_view = crafter_env.get_local_view()


    # To save data
    image_dataset = np.zeros((num_pairs, 2, 1, 64, 64, 3), dtype=np.uint8)
    inventory_dataset = np.zeros((num_pairs, 2, 1, 16), dtype=np.uint8)
    terrain_dataset = np.zeros((num_pairs, 2, 1, 9, 7), dtype=np.uint8)
    objects_dataset = np.zeros((num_pairs, 2, 1, 9, 7), dtype=np.uint8)
    daylight_dataset = np.zeros((num_pairs, 2, 1), dtype=np.float32)
    sleep_dataset = np.zeros((num_pairs, 2, 1), dtype=bool)



    img_dir = f"500k_crafter_data" #f"crafter_test"
    ...
    for pair_i in range(num_pairs):
        ...
        render1 = crafter_env.render_from_glyphs(texture_ids=terrain_buffer[rollout1, t1, ...],
        ...
        # add to dataset
        for k_i, k in enumerate(items):
            inventory_dataset[pair_i, 0, 0, k_i] = sampled_iventory1[k] if k in sampled_iventory1.keys() else 0
        daylight_dataset[pair_i, 0, 0] = daylight_buffer[rollout1, t1]
        sleep_dataset[pair_i, 0, 0] = sleeping_buffer[rollout1, t1]
        terrain_dataset[pair_i, 0, 0, :, :] = terrain_buffer[rollout1, t1, ...]
        objects_dataset[pair_i, 0, 0, :, :] = objects_buffer[rollout1, t1, ...]
        image_dataset[pair_i, 0, 0, :, : :] = render1
        ...
        render2 = crafter_env.render_from_glyphs(texture_ids=terrain_buffer[rollout2, t2, ...],
        ...
        # add to dataset
        for k_i, k in enumerate(items):
            inventory_dataset[pair_i, 1, 0, k_i] = sampled_iventory2[k] if k in sampled_iventory2.keys() else 0
        daylight_dataset[pair_i, 1, 0] = daylight_buffer[rollout2, t2]
        sleep_dataset[pair_i, 1, 0] = sleeping_buffer[rollout2, t2]
        terrain_dataset[pair_i, 1, 0, :, :] = terrain_buffer[rollout2, t2, ...]
        objects_dataset[pair_i, 1, 0, :, :] = objects_buffer[rollout2, t2, ...]
        image_dataset[pair_i, 1, 0, :, :, :] = render2

        #save on drive?
        if saveimg:
            canvas1 = Image.fromarray(render1)
            ...
            canvas2 = Image.fromarray(render2)
    ...
    with open(
            os.path.join(img_dir, f"images.pickle"), "wb"
    ) as handle:
        pickle.dump(
            image_dataset, handle, protocol=pickle.HIGHEST_PROTOCOL
        )

    np.save(
        os.path.join(img_dir, "inventory.npy"),
        inventory_dataset,
    )
    np.save(
        os.path.join(img_dir, "sleep.npy"),
        sleep_dataset,
    )
    np.save(
        os.path.join(img_dir, "daylight.npy"),
        daylight_dataset,
    )
    np.save(
        os.path.join(img_dir, "objects.npy"),
        objects_dataset,
    )
    np.save(
        os.path.join(img_dir, "terrain.npy"),
        terrain_dataset,
    )
