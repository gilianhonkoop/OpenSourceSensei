...
if __name__ == "__main__":
    ...
    run_name = "test_minihack_Plan2Explore_datasets_tasks_keychest"
    job_id = 0
    data_dir = (
    ...
    if saveimg:
        img_dir = f"dreamer_minihack_{run_name}_{job_id}"
    ...
    for pair_i in range(num_pairs):
        ...
        if saveimg:
            ...
            img_concat.save(
                os.path.join(
                    img_dir_vertical, f"pair_{pair_i}_key_first{key_in_inventory1}_second{key_in_inventory2}.png"
                )
            )
