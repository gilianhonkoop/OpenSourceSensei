...
motif_model = motif_pokemon_gpt_gen1
...
pytorch_total_params = sum(p.numel() for p in motif_reward_model.parameters() if p.requires_grad)
print("Total trainable parameters in the motif network: ", pytorch_total_params)

pytorch_total_params = sum(p.numel() for p in motif_reward_model.parameters())
print("Total parameters in the motif network: ", pytorch_total_params)


current_model = motif_model["model"]
...
images = [np.array(Image.open(f)) for f in image_files[1300:2070]]

# Example: 100-step trajectory
# trajectory = np.cumsum(np.random.randn(ep_length, 2), axis=0)
# assert len(images) == len(trajectory), "Image count must match trajectory length"
...
for image_seq in images:
    ...
    reward_dict = reward_model_with_mb(current_model, batch_dict)

    motif_reward = reward_dict.rewards.squeeze().detach().cpu().numpy()
    trajectory.append(motif_reward)

trajectory = np.asarray(trajectory).reshape(-1, 1)

print(trajectory.shape)
print(len(images))

# Set up the figure
fig, (ax_img, ax_plot) = plt.subplots(1, 2, figsize=(12, 4), gridspec_kw={'width_ratios': [1, 4]})
...
line, = ax_plot.plot([], [], 'bo-', lw=2, markersize=2, color=newpurple)
ax_plot.set_ylim(np.min(trajectory[:, 0]) - 1, np.max(trajectory[:, 0]) + 1)
ax_plot.set_xlim(0, len(trajectory))  # y is time
...
# Save to GIF
# ani.save(f'trajectory_gen2_wlevel9_rollout_gymleader_{ep_length}.gif', writer='pillow', fps=20)
