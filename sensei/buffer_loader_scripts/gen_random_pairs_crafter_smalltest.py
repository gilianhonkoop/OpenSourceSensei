...
if __name__ == "__main__":
    ...
    # To save data
    image_dataset = np.zeros((num_pairs, 2, 512, 512, 3), dtype=np.uint8)
    inventory_dataset = np.zeros((num_pairs, 2, 16), dtype=np.uint8)
    terrain_dataset = np.zeros((num_pairs, 2, 9, 7), dtype=np.uint8)
    objects_dataset = np.zeros((num_pairs, 2, 9, 7), dtype=np.uint8)
    daylight_dataset = np.zeros((num_pairs, 2), dtype=np.float32)
    sleep_dataset = np.zeros((num_pairs, 2), dtype=bool)



    img_dir = f"dataset_test" #f"crafter_test"
    ...
    for pair_i in range(num_pairs):
        ...
        for k_i, k in enumerate(items):
            inventory_dataset[pair_i, 0, k_i] = sampled_iventory1[k] if k in sampled_iventory1.keys() else 0
        daylight_dataset[pair_i, 0] = daylight_buffer[rollout1, t1]
        sleep_dataset[pair_i, 0] = sleeping_buffer[rollout1, t1]
        terrain_dataset[pair_i, 0, :, :] = terrain_buffer[rollout1, t1, ...]
        objects_dataset[pair_i, 0, :, :] = objects_buffer[rollout1, t1, ...]
        #image_dataset[pair_i, 0, :, : :] = render1
        ...
        # add to dataset
        for k_i, k in enumerate(items):
            inventory_dataset[pair_i, 1, k_i] = sampled_iventory2[k] if k in sampled_iventory2.keys() else 0
        daylight_dataset[pair_i, 1] = daylight_buffer[rollout2, t2]
        sleep_dataset[pair_i, 1] = sleeping_buffer[rollout2, t2]
        terrain_dataset[pair_i, 1, :, :] = terrain_buffer[rollout2, t2, ...]
        objects_dataset[pair_i, 1, :, :] = objects_buffer[rollout2, t2, ...]
        #image_dataset[pair_i, 1, :, :, :] = render2
    ...
    #np.save(
    #    os.path.join(img_dir, "images.npy"),
    #    image_dataset,
    #)
    np.save(
