...
class Textures:
  ...
  def __init__(self, directory):
    ...
    for filename in pathlib.Path(directory).glob('*.png'):
      ...
      texture_names.append(filename.stem)
    self.texture_names = sorted(texture_names)
...
class LocalView:
  ...
  def __init__(self, world, textures, grid):
    ...
    self._texture_dict = {x: i for i, x in enumerate([None] + textures.texture_names)}
    self._texture_rev_dict = {i: x for i, x in enumerate([None] + textures.texture_names)}
  ...
  def __call__(self, player, unit):
    ...
    texture_ids = np.zeros((self._grid[0], self._grid[1]), dtype=np.uint8)
    obj_ids = np.zeros((self._grid[0], self._grid[1]), dtype=np.uint8)
    for x in range(self._grid[0]):
      for y in range(self._grid[1]):
        ...
        texture_ids[x, y] = self._texture_dict[self._world[pos][0]]
    ...
    for obj in self._world.objects:
      ...
      obj_ids[pos[0], pos[1]] = self._texture_dict[obj.texture]
    ...
    return canvas, texture_ids, obj_ids, self._world.daylight, player.sleeping

  def get_canvas(self, unit, texture_ids, obj_ids,  daylight, sleeping):
    canvas = np.zeros(tuple(self._grid * unit) + (3,), np.uint8) + 127
    for x in range(self._grid[0]):
      for y in range(self._grid[1]):
        #pos = self._center + np.array([x, y]) - self._offset
        #if not _inside((0, 0), pos, self._area):
        #  continue
        _draw(canvas, np.array([x, y]) * unit,  self._textures.get(self._texture_rev_dict[texture_ids[x, y]], unit))
        if obj_ids[x, y]:
          _draw_alpha(canvas,  np.array([x, y]) * unit, self._textures.get(self._texture_rev_dict[obj_ids[x, y]], unit))
    canvas = self._light(canvas, daylight)
    if sleeping:
      canvas = self._sleep(canvas)
    return canvas
