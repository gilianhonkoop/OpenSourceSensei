...
class Env(BaseClass):
  ...
  def render(self, size=None):
    ...
    local_view, _,_,_,_ = self._local_view(self._player, unit)
  ...
  def render_from_glyphs(self, texture_ids, object_ids, daylight, sleeping, inventory, size=None,):
    size = size or self._size
    unit = size // self._view
    canvas = np.zeros(tuple(size) + (3,), np.uint8)
    local_view = self._local_view.get_canvas(unit, texture_ids, object_ids,daylight, sleeping)
    item_view = self._item_view(inventory, unit)
    view = np.concatenate([local_view, item_view], 1)
    border = (size - (size // self._view) * self._view) // 2
    (x, y), (w, h) = border, view.shape[:2]
    canvas[x: x + w, y: y + h] = view
  ...
  def get_glyphs(self):
    ...
    _, terrain, objects, daylight, sleep = self._local_view(self._player, unit)
    return terrain, objects, daylight, sleep
