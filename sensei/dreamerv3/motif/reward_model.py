...
class RewardModel(nn.Module):
    ...
    def forward(self, mb, add_dim=False):
        for key, value in mb.items():
            if key in ["observations", "clip_embedding_cam_right", "clip_embedding_cam_left", "images"]:
                if add_dim:
        ...
        torch.save(
            {
                ...
                **(
                    ...
                    if self.encoder.normalize_inputs and self.encoder.use_clip_embedding_right
                    else {}
                ),
                **(
                    {"clip_left_normalizer": self.encoder.clip_left_normalizer.state_dict()}
                    if self.encoder.normalize_inputs and self.encoder.use_clip_embedding_left
                    else {}
                ),
            },
            path,
    ...
    def load(self, path, device= None):
        ...
        if self.encoder.normalize_inputs:
            if self.encoder.use_obs_vec:
                self.encoder.obs_vec_normalizer.load_state_dict(state_dicts["obs_vec_normalizer"])
            if self.encoder.use_clip_embedding_right:
                self.encoder.clip_right_normalizer.load_state_dict(state_dicts["clip_right_normalizer"])
            if self.encoder.use_clip_embedding_left:
                self.encoder.clip_left_normalizer.load_state_dict(state_dicts["clip_left_normalizer"])
