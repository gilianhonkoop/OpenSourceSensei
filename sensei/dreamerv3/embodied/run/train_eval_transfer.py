...
def train_eval_transfer(old_agent, train_env, eval_env, train_replay, eval_replay, logger, cleanup, cluster, args):
  ...
  # We load the agent but replace the task policy with an empty policy for transfer learning

  # 1. Create a checkpoint for a newly initialized agent
  dummy_checkpoint = embodied.Checkpoint(logdir / 'dummy_checkpoint.ckpt')
  dummy_checkpoint.step = step
  dummy_checkpoint.agent = old_agent
  dummy_checkpoint.train_replay = train_replay
  dummy_checkpoint.eval_replay = eval_replay
  dummy_checkpoint.save()
  time.sleep(30) # wait for checkpoint to be written

  # 2. We will use the newly initialized modules for all components related to the task_behavior
  empty_agnt_state = dummy_checkpoint.load()['agent']
  replace_agnt_state = {k:v for k,v in empty_agnt_state.items() if 'task_behavior' in k}
  for k in replace_agnt_state.keys():  # debugging
    print(f"Replacing agent module {k}")

  # 3. We load a pretrained agent, but components related to task_behavior are substituted by the new ones
  assert embodied.Path(logdir / 'checkpoint.ckpt').exists(), "Need a pretrained checkpoint for transfer learning."
  old_checkpoint = embodied.Checkpoint(logdir / 'checkpoint.ckpt')
  old_checkpoint.step = step
  old_checkpoint.agent = old_agent
  old_checkpoint.train_replay = train_replay
  old_checkpoint.eval_replay = eval_replay
  if args.from_checkpoint:
    old_checkpoint.load(args.from_checkpoint)
  old_checkpoint.load(replace_key='agent', replace_state=replace_agnt_state)

  # 4. Finally we create a new checkpoint to which we can save
  agent = old_agent
  checkpoint = embodied.Checkpoint(logdir / 'new_checkpoint.ckpt')
  ...
  def per_episode(ep, step, mode):
    ...
    for key in args.log_keys_video:
      if key in ep:
        if (mode == 'train' and should_log_trainpolicy(step)) or mode == 'eval':  # we log every eval
  ...
  policy_train = lambda *args: agent.policy(*args, mode='train') # no more eplore
