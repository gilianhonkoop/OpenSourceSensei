...
def train_finetune_eval(
    old_agent, agent_const, train_env, eval_env, train_replay, eval_replay, logger, cleanup, cluster, args):
  ...
  empty_state = dummy_checkpoint.load()
  for k in empty_state['agent'].keys():
    print(f"Empty key for {k}")

  replace_state = {}
  for k, v in empty_state['agent'].items():
    if 'task_behavior' in k:
      replace_state[k] = v

  for k in replace_state.keys():
    print(f"Replacement key for {k}")
  ...
  old_checkpoint.load(replace_key='agent', replace_state=replace_state)

  #assert False,

  #print("Get agent =", old_agent.agent)
  #print("Get wm =", old_agent.agent.wm)

  agent = old_agent #agent_const(old_agent.agent)
  # loaded agent, now we overwrite the policy
  checkpoint = embodied.Checkpoint(logdir / 'new_checkpoint.ckpt')
  checkpoint.step = step
  checkpoint.agent = agent
  checkpoint.train_replay = train_replay
  checkpoint.eval_replay = eval_replay
  checkpoint.save() # ovewrite
