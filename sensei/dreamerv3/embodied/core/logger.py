...
class TensorBoardOutput(AsyncOutput):
  ...
  def __init__(self, logdir, config, fps=20, maxsize=1e9, parallel=True):
    super().__init__(self._write, parallel)
    self._logdir = str(logdir)
    self._ubuntu_os = config.cluster=='mpi' # CLUSTER_UTILS
  ...
  def _video_summary(self, name, video, step):
    ...
    try:
      ...
      image.encoded_image_string = _encode_gif(video, self._fps, ubuntu_ffmpeg=self._ubuntu_os)
