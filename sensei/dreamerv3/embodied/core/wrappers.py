...
class DummyMotif(base.Wrapper):
  def __init__(self, env, key='motif_reward', dummy_value=0.0):
    super().__init__(env)
    self._key = key
  ...
  def obs_space(self):
    spaces = self.env.obs_space
    spaces[self._key] = spacelib.Space(np.float32, shape=(1,))
    return spaces

  def step(self, action):
    obs = self.env.step(action)
    obs[self._key] = np.array([self._dummy_value])
    return obs
