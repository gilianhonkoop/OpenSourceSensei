...
class RoboDesk(gym.Env):
  ...
  def __init__(self, task='open_slide', reward='dense', action_repeat=1,
               episode_length=500, image_size=64, camera='top', fix_reset=False, log_rewards=False, obs_full_state=True, include_modified_tasks=False):
    ...
    self.log_rewards = log_rewards  # log all rewards as part of the observation
    ...
    self.reward_functions = {
        ...
        'open_slide_easy': self._slide_reward_easy,
        'open_drawer': self._drawer_reward,
        'open_drawer_medium': self._drawer_reward_medium,
        'open_drawer_easy': self._drawer_reward_easy,
    ...
    if include_modified_tasks: 
      self.reward_functions['open_slide_easy'] = self._slide_reward_easy
      self.reward_functions['open_drawer_easy'] = self._drawer_reward_easy
      self.reward_functions['open_drawer_medium'] = self._drawer_reward_medium


    self.reward_type = 'dense_reward' if self.reward == 'dense' else 'success'
  ...
  def observation_space(self):
    spaces = {
        ...
        'hr_image': gym.spaces.Box(0, 255, (120, 120, 3), np.uint8) if self.camera == 'top' else gym.spaces.Box(0, 255, (224, 224, 3), np.uint8), # TODO hardcoded values
    ...
    if self.log_rewards:
        for k in self.reward_functions.keys():
            spaces[f'rew_{k}'] = gym.spaces.Box(-np.inf, np.inf, (), np.float32)
    if self.obs_full_state:
        spaces['state'] = gym.spaces.Box(-np.inf, np.inf, (73,), np.float64)
  ...
  def render(self, mode='rgb_array', resize=True):
    if self.camera == 'top':
        ...
        params = {'distance': 1.55, 'azimuth': 180, 'elevation': -18, 'size': 224}
        lookat = [-0.13597396, 0.535, 0.64500556]
    else:
    ...
    camera._render_camera.lookat[:] = lookat  # pylint: disable=protected-access

    image = camera.render(depth=False, segmentation=False)
    camera._scene.free()  # pylint: disable=protected-access

    self.hr_image = np.asarray(image)
    if resize:
      image = Image.fromarray(image)
      if 'crop_box' in params.keys():
  ...
  def _get_init_robot_pos(self):
    ...
    if self.fix_reset:
        low_rg = self.physics.model.actuator_ctrlrange[:self.num_joints, 0]
        high_rg = self.physics.model.actuator_ctrlrange[:self.num_joints, 1]
        init_joint_pose += 0.15 * (low_rg + 0.5 * (high_rg - low_rg))
    else:
  ...
  def reset(self):
    ...
    if not self.fix_reset:
      ...
      self.physics.named.data.qpos['upright_block'][0] += (0.3 * np.random.random() + 0.05)
      self.physics.named.data.qpos['upright_block'][1] += (0.05 * np.random.random())
    else:
      self.physics.named.data.qpos['drawer_joint'] -= 0.10 * 0.5
      self.physics.named.data.qpos['slide_joint'] += 0.20 * 0.5

      self.physics.named.data.qpos['flat_block'][0] += 0.3 * 0.5
      self.physics.named.data.qpos['flat_block'][1] += 0.07 * 0.5
      self.physics.named.data.qpos['ball'][0] += 0.48 * 0.5
      self.physics.named.data.qpos['ball'][1] += 0.08 * 0.5
      self.physics.named.data.qpos['upright_block'][0] += (0.3 * 0.5 + 0.05)
      self.physics.named.data.qpos['upright_block'][1] += (0.05 * 0.5)
  ...
  def _slide_reward_easy(self, reward_type='dense_reward'):
    blocks = ['flat_block', 'upright_block', 'ball']
    if reward_type == 'dense_reward':
      return self._slide_reward(reward_type)
    elif reward_type == 'success':
      return 1 * (self.physics.named.data.qpos['slide_joint'] > 0.45)
  ...
  def _drawer_reward_medium(self, reward_type='dense_reward'):
    if reward_type == 'dense_reward':
      return self._drawer_reward(reward_type)
    elif reward_type == 'success':
      return 1 * (self.physics.named.data.qpos['drawer_joint'] < -0.15)

  def _drawer_reward_easy(self, reward_type='dense_reward'):
    if reward_type == 'dense_reward':
      return self._drawer_reward(reward_type)
    elif reward_type == 'success':
      return 1 * (self.physics.named.data.qpos['drawer_joint'] < -0.1)
  ...
  def _get_obs(self):
    ...
    if self.log_rewards:
      for k,v in self.reward_functions.items():
        obs_dict[f'rew_{k}'] = float(v('success'))  # only sparse rewards here 
      obs_dict.update(self._get_interaction(obs_dict['qvel_objects']))
    if self.obs_full_state:
        obs_dict["state"] = self.physics.get_state()
    return obs_dict

  def _get_interaction(self, qvel_objects):
    interaction_dict = {}
    qvel_objects_inds = {
      "drawer_joint": 0,
      "slide_joint": 1, 
    ...
    for key, ind in qvel_objects_inds.items():
      new_key = f"rew_interaction_{key}" 
      if "ball" in key or "block" in key:
        interaction_dict[new_key] = np.any(np.abs(qvel_objects[ind])>2*1e-2, axis=-1)*1
      else:
        interaction_dict[new_key] = (np.abs(qvel_objects[ind])>2*1e-2) * 1
