...
class RewardModelTrainer:
    ...
    def train(self, save_cpt=False, working_dir=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, mb in enumerate(tqdm.tqdm(train_loader)):
                if num_iter % len(train_loader) == 0:
                    ...
                    self.writer.add_scalar(
                        "validation/epoch_loss", val_met.total_val_loss[-1], num_iter
                    )
                    self.writer.add_scalar(
                        "validation/validation_accs",
                        val_met.validation_accs[-1],
                        num_iter,
                    )
                    # save(
                    #     cfg,
            ...
            self.writer.add_scalar(
                "train/train_accs", train_acc / len(train_loader), num_iter
            )
        ...
        torch.save(
            {
                ...
                **(
                    ...
                    if self.reward_model.encoder.normalize_inputs
                    and self.reward_model.encoder.use_obs_vec
                    else {}
                ),
                **(
                    {
                    ...
                    if self.reward_model.encoder.normalize_inputs
                    and self.reward_model.encoder.use_clip_embedding_right
                    else {}
                ),
                **(
                    {
                    ...
                    if self.reward_model.encoder.normalize_inputs
                    and self.reward_model.encoder.use_clip_embedding_left
                    else {}
                ),
            },
            path,
