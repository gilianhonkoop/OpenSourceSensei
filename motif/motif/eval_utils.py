...
def get_batch_dict_for_reward_model(
    reward_model, rollout_images=None, rollout_obs=None, rollout_images_left=None
):
    mb = {}
    if reward_model.encoder.use_obs_vec:
        assert rollout_obs is not None
        mb["observations"] = (
            torch.from_numpy(rollout_obs).float().to(reward_model.device)
        )
    if reward_model.encoder.use_image:
        ...
        mb["images"] = images
    if (
        reward_model.encoder.use_clip_embedding_right
        or reward_model.encoder.use_clip_embedding_left
    ):
